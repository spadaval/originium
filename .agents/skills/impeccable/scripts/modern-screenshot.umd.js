...
`);
  ...
  function Se(e, t) {
    ...
    const r = e.cloneNode(!1),
      ...
      i = r.getContext("2d");
    try {
      return n && i?.putImageData(n.getImageData(0, 0, e.width, e.height), 0, 0), r;
    } catch (a) {
      t.log.warn("Failed to clone canvas", a);
    }
    return r;
  ...
  function z(e, t) {
    ...
    let E = u.get(r);
    if (!E) {
      m && m instanceof RegExp && m.test(s) && (s += (/\?/.test(s) ? "&" : "?") + Date.now());
  ...
  function $t(e, t) {
    const r = [];
    return e.replace(ke, (n, _i, a) => (r.push([a, ue(a, t)]), n)), r.filter(([n]) => !re(n));
  }
  function Lt(e) {
              ...
              Array.from(f.cssRules).map((h) =>
                C(this, null, function* () {
                  if (Je(h)) {
                    ...
                    const E = p.replace(ke, (b, _w, S) => b.replace(S, ue(S, g)));
                    for (const b of Ht(E))
                      try {
